...
<div id="pubcol1">
    <div id="pubbarracompartir">
        ...
        <a class="sharelink" target="_blank" href="imprimir.php?tipo=clasic&id=<?=$r->id ?>">PRINT IT</a>
        <a class="sharelink" target="_blank" href="<?= $r->mapa ?>">MAP</a>
    </div>
    <div class="compartir" id="cuadroshare">
        <?php
        $miurl=rawurlencode("http://www.mad24.es?cat=".$_REQUEST["cat"]."&type=clasic&id=".$r->id);
        ?>
    ...
    <div class="compartir" id="cuadromail">
        <form action="#" name="frmsharemail" id="frmsharemail">
            ...
            <div class="filamail">
                <input type="submit" class="submitmail" value="Send" />
            </div>
        </form>
    </div>
    <div id="pubgaleriafotos">
        <?php
        $imgs=explode(",",$r->galeriaclasic);
        if(count($imgs)>0) {
            $cimg=1;
            $csep=1;
            $cad1="";
    ...
    <div id="pubgaleriapag">
        ...
        <?php } ?>
    <div id="pubrateit">
        <span class="vovlervideo" onclick="volveralvideo()">&lt; Back to Image</span>
        Rate It!
...
<div id="pubcol2">
    <div id="pubvideoimagen">
        <img id="flashplus" src="<?="images/videoplus/".$r->videofichaclasic; ?>" border="0" />
        <img id="pubimgdelante" src="" border="0" style="display:none;" />
        <img id="pubimgatras"   src="" border="0" style="display:none"  />
    </div>
    <div id="publogo">
        <?php echo '<img src="images/logoplus/'.$r->logofichaclasic.'" border="0" />' ?>
    </div>
    ...
    <div id="pubcucomentario">
        ...
        <form id="frmcomentario" name="frmcomentario" onsubmit="return validacomentario()" method="post" enctype="multipart/form-data">
            ...
            <input type="hidden" name="pubtipo" value="clasic" />
        ...
        $("#pubimgdelante").css({
            ...
            if($("#pubimgdelante").attr("width")<=50 || $("#pubimgdelante").attr("height")<=50){
                if($("#pubimgatras").attr("width")<=50 || $("#pubimgatra").attr("height")<=50){
                    $("#pubvideoimagen").css("width",459);
                    $("#pubvideoimagen").css("height",273);
                }else{
