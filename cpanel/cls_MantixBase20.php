...
function insertar()
{
 ...
 switch($this->datos->c_error) {
   ...
   case 16: return mysql_error(); break;
...
function actualizar()
{
 ...
 $this->post_upd();
 if($this->datos->c_error!=0) { return mysql_error(); }
...
function eliminar_simple()
{
   ...
   if($cad!="") { return "No se pudo eliminar debido a que existe el valor en ".$cad; }
   else {
...
function activar_simple()
{
 ...
 if($this->datos->c_error!=0) { return "Se encontr&oacute; un Problema al Intentar Activar el Registro. Por favor, comun&iacute;quese con Soporte."; }
 { return "Activaci&oacute;n realizada con &eacute;xito"; }
}
function desactivar_simple()
{
   $this->datos->desactivar_simple($this->id);
 if($this->datos->c_error!=0) { return "Se encontr&oacute; un Problema al Intentar Desactivar el Registro. Por favor, comun&iacute;quese con Soporte."; }
...
function eliminar()
{
   ...
   if(isset($fk)) {
	   ...
	   $scad=implode(",",$cad);
	   if($scad!="")  { return "No se pudo eliminar alguno o todos los registros debido a que existe el valor en ".$scad; }
